...
pub async fn get_broker_handler(broker_pk: i64, user: User) -> Result<impl Reply, Rejection> {
    ...
    let broker_detailed = run_repeatable_read_transaction(&mut connection, async |connection| {
        ...
        let is_admin = owner.pk == user.pk || is_broker_admin(broker.pk, &user, connection).await?;
        ...
        let quota_bytes = if is_unlimited {
            ...
            broker::table
                .inner_join(broker_access::table.on(
                    get_broker_access_condition!(user.pk).and(broker_access::quota.is_not_null()),
                ))
                .group_by(broker::pk)
