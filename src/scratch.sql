WITH cte5 AS (
                ...
                LEFT JOIN post_tag ON post_tag.fk_post = post.pk 
                GROUP BY post.pk
                HAVING (array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte0)) AND (
                    array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte1) 
                    OR array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte2) 
                    OR array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte3) 
                    OR array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte4) 
                    OR array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte5) 
                )
            ), countCte AS (
                SELECT 
                CASE WHEN (SELECT COUNT(*) FROM (SELECT post.pk FROM post LEFT JOIN s3_object ON s3_object.object_key = post.s3_object INNER JOIN registered_user create_user ON create_user.pk = post.fk_create_user WHERE post.pk IN (SELECT pk FROM postPks) LIMIT 100000) limitedPks) < 100000 
                THEN (SELECT COUNT(*) FROM (SELECT post.pk FROM post LEFT JOIN s3_object ON s3_object.object_key = post.s3_object INNER JOIN registered_user create_user ON create_user.pk = post.fk_create_user WHERE post.pk IN (SELECT pk FROM postPks) AND
            ...
            LEFT JOIN s3_object ON s3_object.object_key = post.s3_object 
            INNER JOIN registered_user create_user ON create_user.pk = post.fk_create_user 
            WHERE post.pk IN (SELECT pk FROM postPks) AND 
            (post.fk_create_user = 1
            OR (post.public AND TRUE)
            OR EXISTS(
                SELECT * FROM post_group_access
            ...
            ORDER BY post.pk DESC LIMIT LEAST(50, 100) OFFSET (50) * 14;
...
WITH cte5 AS (
                ...
                CASE WHEN (SELECT COUNT(*) FROM (SELECT DISTINCT post.pk FROM post LEFT JOIN s3_object ON s3_object.object_key = post.s3_object INNER JOIN registered_user create_user ON create_user.pk = post.fk_create_user LEFT JOIN post_tag ON post_tag.fk_post = post.pk GROUP BY post.pk HAVING (array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte0)) AND (
                    ...
                    OR array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte4) 
                    OR array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte5) 
                ) LIMIT 100000) limitedPks) < 100000 
            ...
            )) GROUP BY post.pk HAVING (array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte0)) AND (
                    array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte1) 
                    OR array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte2) 
                    OR array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte3) 
                    OR array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte4) 
...
WITH cte0 AS (
                        ...
                        SELECT CASE WHEN (SELECT COUNT(*) FROM (SELECT DISTINCT post.pk FROM post LEFT JOIN s3_object ON s3_object.object_key = post.s3_object INNER JOIN registered_user create_user ON create_user.pk = post.fk_create_user LEFT JOIN post_tag ON post_tag.fk_post = post.pk GROUP BY post.pk HAVING array_agg(post_tag.fk_post) && ARRAY(SELECT tag_keys FROM cte0) AND array_agg(post_tag.fk_post) && ARRAY(SELECT tag_keys FROM cte1) LIMIT 100000) limitedPks) < 100000 
                        THEN (SELECT COUNT(*) FROM (SELECT DISTINCT post.pk FROM post LEFT JOIN s3_object ON s3_object.object_key = post.s3_object INNER JOIN registered_user create_user ON create_user.pk = post.fk_create_user LEFT JOIN post_tag ON post_tag.fk_post = post.pk WHERE 
            (post.fk_create_user = 1
            OR (post.public AND TRUE)
            OR EXISTS(
            ...
            )) GROUP BY post.pk HAVING array_agg(post_tag.fk_post) && ARRAY(SELECT tag_keys FROM cte0) AND array_agg(post_tag.fk_post) && ARRAY(SELECT tag_keys FROM cte1)) pks) END AS full_count
            ), postPks AS (SELECT post.pk FROM post LEFT JOIN s3_object ON s3_object.object_key = post.s3_object INNER JOIN registered_user create_user ON create_user.pk = post.fk_create_user LEFT JOIN post_tag ON post_tag.fk_post = post.pk WHERE 
            (post.fk_create_user = 1
            OR (post.public AND TRUE)
            OR EXISTS(
                SELECT * FROM post_group_access
...
LEFT JOIN post_tag ON post_tag.fk_post = post.pk 
GROUP BY post.pk 
HAVING array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte0) AND array_agg(post_tag.fk_tag) && ARRAY(SELECT tag_keys FROM cte1) 
ORDER BY post.pk DESC 
LIMIT LEAST(50, 100) OFFSET (50) * 0;
...
WHERE EXISTS(
    ...
    WHERE post_tag.fk_post = post.pk 
    AND post_tag.fk_tag IN (SELECT tag_keys FROM cte0)
) AND EXISTS(
    SELECT * FROM post_tag 
    WHERE post_tag.fk_post = post.pk 
    AND post_tag.fk_tag IN (SELECT tag_keys FROM cte1)
)
ORDER BY post.pk DESC 
LIMIT LEAST(50, 100) OFFSET (50) * 0
