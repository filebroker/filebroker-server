        ...
        run_serializable_transaction(&mut connection, |connection| {
            async move {
                ...
                let (create_user, edit_user) = post::table
                    .inner_join(
                        create_user
                            .on(post::fk_create_user.eq(create_user.field(registered_user::pk))),
                    )
                    .inner_join(
                ...
                let post_detailed = PostDetailed {
                    pk: post.pk,
                ...
                let apply_auto_tags_task =
                    create_apply_auto_tags_for_post_task(post.pk, connection).await?;

                Ok((post_detailed, apply_auto_tags_task))
            }
            .scope_boxed()
        })
        .await?;
            ...
            async move {
                ...
                if post_collection.poster_object_key.is_none() {
                    ...
                    if let Some(thumbnail_object) = thumbnail_object {
                        diesel::update(post_collection::table)
                            ...
                            .set(
                                post_collection::poster_object_key.eq(&thumbnail_object.object_key),
                            )
                            .execute(connection)
                            .await?;
                    }
                ...
                let (create_user, edit_user) = post_collection::table
                    .inner_join(create_user.on(
                        post_collection::fk_create_user.eq(create_user.field(registered_user::pk)),
                    ))
                    .inner_join(
                        edit_user
                            .on(post_collection::fk_edit_user
                                .eq(edit_user.field(registered_user::pk))),
                    )
                    .select((
                        create_user.fields(registered_user::table::all_columns()),
                        edit_user.fields(registered_user::table::all_columns()),
                ...
                let post_collection_detailed = PostCollectionDetailed {
                ...
                let apply_auto_tags_task =
                    create_apply_auto_tags_for_collection_task(post_collection.pk, connection)
                        .await?;

                Ok((post_collection_detailed, apply_auto_tags_task))
            }
            .scope_boxed()
        })
        .await?;
