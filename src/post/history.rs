...
pub async fn rewind_post_history_snapshot_handler(
    ...
    let mut connection = acquire_db_connection().await?;
    let post = run_serializable_transaction(&mut connection, async |connection| {
        ...
        let mut snapshot_tags = get_post_snapshot_tags(&post_snapshot, &post, connection).await?;
            ...
            removed_groups = diesel::delete(
                post_group_access::table.filter(
                    post_group_access::fk_post
                        .eq(post.pk)
                        .and(post_group_access::fk_granted_group.eq_any(group_access_to_remove)),
                ),
            )
            .execute(connection)
            .await
...
pub async fn rewind_post_collection_history_snapshot_handler(
    ...
    let mut connection = acquire_db_connection().await?;
    let post_collection = run_serializable_transaction(&mut connection, async |connection| {
        ...
        let curr_group_access =
            get_post_collection_group_access(post_collection.pk, Some(&user), connection).await?;
